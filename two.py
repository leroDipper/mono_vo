...
for filename in sorted(os.listdir(images_path))[:115]:
...
if marker_poses:
    ...
    # Plot results with start/end markers
    fig, ((ax1, ax2), (ax3, ax4)) = plt.subplots(2, 2, figsize=(12, 10))
    
    # XY trajectory with start/end markers
    ax1.plot(poses[:, 0], poses[:, 1], 'b-', linewidth=1)
    ax1.scatter(poses[0, 0], poses[0, 1], c='green', s=100, marker='o', label='START')
    ax1.scatter(poses[-1, 0], poses[-1, 1], c='red', s=100, marker='x', label='END')
    ax1.set_xlabel('X (m)')
    ax1.set_ylabel('Y (m)')
    ax1.set_title('ArUco Trajectory (X-Y)')
    ax1.legend()
    ax1.axis('equal')
    ax1.grid(True)
    ...
    # XZ trajectory with start/end markers
    ax3.plot(poses[:, 0], poses[:, 2], 'g-', linewidth=1)
    ax3.scatter(poses[0, 0], poses[0, 2], c='green', s=100, marker='o', label='START')
    ax3.scatter(poses[-1, 0], poses[-1, 2], c='red', s=100, marker='x', label='END')
    ax3.set_xlabel('X (m)')
    ax3.set_ylabel('Z (m)')
    ax3.set_title('ArUco Trajectory (X-Z)')
    ax3.legend()
    ax3.grid(True)
