...
# --- Initialize pose from first detected marker ---
trajectory = []
...
for idx, filename in enumerate(image_files[:5]):
    ...
    if ids is not None and 0 in ids.flatten():
        ...
        print(f"Initialized VO from ArUco at frame {idx}, pos={camera_pos}")
        break
...
for idx, filename in enumerate(image_files[:122]):
    ...
    # DEBUG: Check essential matrix quality
    if idx < 25:
        inliers = np.sum(mask)
        ...
        if ids is not None and 0 in ids.flatten():
            ...
            if len(marker_positions) == 2:
                ...
                if vo_dist > 1e-6:
                    ...
                    print("Now using PURE VO for all subsequent frames")
